...
# ===================== GO/STOP CONSTANTS =====================
pygame.display.set_caption("GO / STOP")
...
MAX_ALERT = 100.0
FALSE_SAFETY_TIME = 0.7

MOVEMENT_THRESHOLD_MICRO = 0.015
MOVEMENT_THRESHOLD_SLOW = 0.035
MOVEMENT_THRESHOLD_SHARP = 0.08

ALERT_INCREASE_MICRO = 3.0
ALERT_INCREASE_SLOW = 12.0
ALERT_INCREASE_SHARP = 35.0
ALERT_DECAY_NORMAL = 4.0
ALERT_DECAY_FAST = 10.0
FREEZE_TIME_FOR_FAST_DECAY = 1.2
...
def get_landmark_points(landmarks):
    return [(lm.x, lm.y) for lm in landmarks.landmark] if landmarks else None


def detect_movement_type(prev_lm, curr_lm):
    if prev_lm is None or curr_lm is None or len(prev_lm) == 0:
        return 0
    diffs = [np.linalg.norm(np.array(p) - np.array(c)) for p, c in zip(prev_lm, curr_lm)]
    max_diff = float(np.max(diffs))
    avg_diff = float(np.mean(diffs))
    if max_diff > MOVEMENT_THRESHOLD_SHARP:
        return 3
    if avg_diff > MOVEMENT_THRESHOLD_SLOW:
        return 2
    if max_diff > MOVEMENT_THRESHOLD_MICRO:
        return 1
    return 0
...
def instructions_screen():
    ...
    lines = [
        "1. Гра складається з циклів: MOVE → SLOW → FREEZE.",
        "2. У синій фазі MOVE можна рухатися.",
        "3. У жовтій фазі SLOW рухайся дуже повільно.",
        "4. У червоній фазі FREEZE завмри, інакше росте тривога.",
        "5. 5 циклів (~5-6 хв): впорайся, щоб не спіймали!"
    ]
...
def run_game():
    ...
    while cap.isOpened() and not game_over:
        ...
        curr_landmarks = get_landmark_points(results.multi_hand_landmarks[0]) if results.multi_hand_landmarks else None
        ...
        if phase == "blue" and time_in_phase >= BLUE_TIME:
            phase = "yellow"
            phase_start = time.time()
        elif phase == "yellow" and time_in_phase >= YELLOW_TIME:
            phase = "red"
            phase_start = time.time()
        elif phase == "red" and time_in_phase >= RED_TIME:
            phase = "blue"
            phase_start = time.time()
            current_cycle += 1
            if current_cycle >= CYCLES:
                win = alert_level < MAX_ALERT
                game_over = True

        # Движение
        move_type = detect_movement_type(prev_landmarks, curr_landmarks)
        is_detection_active = (phase in ["yellow", "red"]) and (time_in_phase > FALSE_SAFETY_TIME or phase == "yellow")

        if is_detection_active and curr_landmarks:
            if move_type > 0:
                increase = (
                    ALERT_INCREASE_MICRO if move_type == 1 else
                    ALERT_INCREASE_SLOW if move_type == 2 else
                    ALERT_INCREASE_SHARP
                )
                alert_level = min(MAX_ALERT, alert_level + increase)
                freeze_start = None
            else:
                if freeze_start is None:
                    freeze_start = time.time()
                elapsed = time.time() - freeze_start
                decay = ALERT_DECAY_FAST if elapsed >= FREEZE_TIME_FOR_FAST_DECAY else ALERT_DECAY_NORMAL
                alert_level = max(0, alert_level - decay * (1 / 30))
        else:
            alert_level = max(0, alert_level - ALERT_DECAY_NORMAL / 2 * (1 / 30))
            freeze_start = None
        ...
        if phase == "red":
            pulse = 100 + 100 * np.sin(time.time() * 3)
            overlay = np.full((h, w, 3), COLOR_RED, dtype=np.uint8)
            alpha = (pulse / 255) * 0.3
            frame = cv2.addWeighted(frame, 1, overlay, alpha, 0)
            text = "FREEZE!"
        elif phase == "yellow":
            overlay = np.full((h, w, 3), COLOR_YELLOW, dtype=np.uint8)
            frame = cv2.addWeighted(frame, 1, overlay, 0.25, 0)
            text = "SLOW..."
        else:
            overlay = np.full((h, w, 3), COLOR_BLUE, dtype=np.uint8)
            frame = cv2.addWeighted(frame, 1, overlay, 0.2, 0)
            text = "MOVE!"

        frame = draw_text_pil(frame, text, (w // 2 - 120, h // 2 - 50), 60, COLOR_WHITE)

        bx, by = margin + 80, h - margin - 60
        progress = alert_level / MAX_ALERT
        color_bar = (0, 0, 255) if progress > 0.6 else (0, 200, 255) if progress > 0.3 else (100, 255, 100)
        cv2.rectangle(frame, (bx, by), (bx + 300, by + 20), (40, 40, 40), -1)
        cv2.rectangle(frame, (bx, by), (bx + int(300 * progress), by + 20), color_bar, -1)
        frame = draw_text_pil(frame, "Alert Level", (bx, by - 35), 22, COLOR_WHITE)

        remaining = max(0, {"blue": BLUE_TIME, "yellow": YELLOW_TIME, "red": RED_TIME}[phase] - time_in_phase)
        frame = draw_text_pil(frame, f"Cycle: {current_cycle + 1}/{CYCLES}", (margin + 80, margin + 40), 32, COLOR_WHITE)
        frame = draw_text_pil(frame, f"Phase time: {int(remaining)}s", (margin + 80, margin + 80), 28, COLOR_ACCENT)
    ...
    # Финальный экран
    final_text = "УСПІХ! Збій виконано!" if win else "ОХОРОНА! Ти спійманий!"
    final_color = COLOR_ACCENT if win else COLOR_RED
    frame = draw_text_pil(frame, final_text, (w // 2 - 300, h // 2), 50, final_color)
    frame_rgb = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
    frame_rgb = np.fliplr(frame_rgb)
    frame_rgb = np.rot90(frame_rgb)
    frame_surf = pygame.surfarray.make_surface(frame_rgb)
    ...
    return STATE_MENU if win else STATE_GAME_OVER
